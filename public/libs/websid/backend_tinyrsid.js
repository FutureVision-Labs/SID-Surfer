...
var __tinyrsidBasePath=(function(){try{if(typeof window!=='undefined'){if(window.__TINYSID_BASE_PATH__)return window.__TINYSID_BASE_PATH__;var script=document.currentScript;if(!script){var scripts=document.getElementsByTagName('script');script=scripts.length?scripts[scripts.length-1]:null}if(script&&script.src){return new URL('./',script.src).toString()}}}catch(e){}return ''})();var __tinyrsidResolvePath=function(name){try{return __tinyrsidBasePath?new URL(name,__tinyrsidBasePath).toString():name}catch(e){return name}};
